...
<Warning>
The examples below describe the fixed [source implementation](https://github.com/kyegomez/swarms/blob/master/swarms/structs/batch_agent_execution.py). The published `swarms==15.0.3` package still raises `BatchAgentExecutionError`: omitting `imgs` fails while building the batch, and supplying images fails when unpacking completed futures.

If you use that release, use [`run_agents_with_different_tasks`](#run-agents-with-different-tasks) for 1:1 agent-to-task pairing. Updating to `15.0.3` alone does not include the fix.
</Warning>

Runs a list of agents or callables on a parallel list of tasks. Each `agents[i]` runs `tasks[i]`, with `imgs[i]` when provided. Omitting `imgs` supplies `None` to each call. Like `batched_grid_agent_execution`, this is a 1:1 pairing; neither helper runs every agent on every task.
...
**Parameters:**
- `agents` (List[Agent | Callable]): Agents or callables to run. The helper calls `agent.run(task, img)` when `run` exists, otherwise `agent(task, img)`.
- `tasks` (List[str]): Supply one task per agent, with the same length as `agents`. The default `None` is not a valid task list.
...
**Raises:** `BatchAgentExecutionError` for batch setup or management failures, including mismatched task or image list lengths. Individual agent failures produce `None` as described above.
...
results = batch_agent_execution(agents=agents, tasks=tasks)
for agent, result in zip(agents, results):
    print(f"{agent.agent_name}: {result}")
```

You can check the pairing without making model requests by passing a callable that returns its received arguments. Accept both positional arguments, even when you omit `imgs`:
...
def echo_inputs(task: str, img: Optional[str] = None) -> tuple[str, Optional[str]]:
    return task, img


results = batch_agent_execution(
    agents=[echo_inputs, echo_inputs],
    tasks=["Summarize", "Translate"],
    max_workers=2,
)
assert results == [("Summarize", None), ("Translate", None)]
print(results)
```

Choose the helper by the arguments you need to pass:

| Helper | Pairing | Image input | Failed agent result |
|---|---|---|---|
| `run_agents_concurrently` | Every agent receives the same task | One shared `img` | Exception in that agent's slot |
| `batched_grid_agent_execution` | Each agent receives its corresponding task | No image parameter | Exception in that agent's slot |
| `batch_agent_execution` | Each agent or callable receives its corresponding task | Optional image per agent | `None` in that agent's slot |

All three return results in input order, even when tasks finish in a different order. Batch setup failures, such as mismatched list lengths, raise instead of becoming per-agent results.
